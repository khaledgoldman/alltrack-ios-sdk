...
- (instancetype)init {
    ...
    self.logger = [ALTAlltrackFactory logger];
...
+ (void)sendAdWordsRequest {
    [[ALTAlltrackFactory logger] warn:@"Send AdWords Request functionality removed"];
}
...
+ (void)setTestOptions:(AlltrackTestOptions *)testOptions {
    @synchronized (self) {
        if (testOptions.teardown) {
            ...
            defaultInstance = nil;
            onceToken = 0;
            [ALTAlltrackFactory teardown:testOptions.deleteState];
        }
...
- (void)setTestOptions:(AlltrackTestOptions *)testOptions {
    ...
    if (testOptions.baseUrl != nil) {
        [ALTAlltrackFactory setBaseUrl:testOptions.baseUrl];
    }
    if (testOptions.gdprUrl != nil) {
        [ALTAlltrackFactory setGdprUrl:testOptions.gdprUrl];
    }
    if (testOptions.subscriptionUrl != nil) {
        [ALTAlltrackFactory setSubscriptionUrl:testOptions.subscriptionUrl];
    }
    if (testOptions.timerIntervalInMilliseconds != nil) {
        ...
        [ALTAlltrackFactory setTimerInterval:timerIntervalInSeconds];
    }
    if (testOptions.timerStartInMilliseconds != nil) {
        ...
        [ALTAlltrackFactory setTimerStart:timerStartInSeconds];
    }
    if (testOptions.sessionIntervalInMilliseconds != nil) {
        ...
        [ALTAlltrackFactory setSessionInterval:sessionIntervalInSeconds];
    }
    if (testOptions.subsessionIntervalInMilliseconds != nil) {
        ...
        [ALTAlltrackFactory setSubsessionInterval:subsessionIntervalInSeconds];
    }
    if (testOptions.noBackoffWait) {
        [ALTAlltrackFactory setSdkClickHandlerBackoffStrategy:[ALTBackoffStrategy backoffStrategyWithType:ALTNoWait]];
        [ALTAlltrackFactory setPackageHandlerBackoffStrategy:[ALTBackoffStrategy backoffStrategyWithType:ALTNoWait]];
    }
    if (testOptions.enableSigning) {
        [ALTAlltrackFactory enableSigning];
    }
    if (testOptions.disableSigning) {
        [ALTAlltrackFactory disableSigning];
    }

    [ALTAlltrackFactory setAdServicesFrameworkEnabled:testOptions.adServicesFrameworkEnabled];
}
