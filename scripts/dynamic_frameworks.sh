...
echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Dynamic Frameworks build - START... ${NC}"
...
echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Moving shared schemas to generate dynamic iOS SDK framework using Carthage ... ${NC}"
...
then
  echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Bulding dynamic iOS target with Carthage ... ${NC}"
...
echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Moving shared schemas to generate dynamic tvOS SDK framework using Carthage ... ${NC}"
mv Alltrack.xcodeproj/xcshareddata/xcschemes/AlltrackSdk.xcscheme .
mv AlltrackSdkTv.xcscheme Alltrack.xcodeproj/xcshareddata/xcschemes
echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Done! ${NC}"
...
then
  echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Bulding dynamic tvOS targets with Carthage ... ${NC}"
  arch -x86_64 /bin/bash ./scripts/carthage_xcode.sh build --no-skip-current
  echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Done! ${NC}"
...
mv Alltrack.xcodeproj/xcshareddata/xcschemes/AlltrackSdkTv.xcscheme .
...
then
  ...
  echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Skipping WebBridge SDK framework build ... ${NC}"
fi
...
echo -e "${CYAN}[ALLTRACK][BUILD]:${GREEN} Dynamic Frameworks build - END... ${NC}"
